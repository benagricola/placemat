...
- [ ] **Step 3:** `migration.md` `## To 0.25`: searched parts may move after placement to shorten wire and links; `[cleanup] enabled = false` gives the old placement.
- [ ] **Step 4:** Version 0.25.0 in the three files; reinstall (`uv pip install -e . --no-deps`) so the version test passes.
