...
- [ ] Failing tests from the router's own cases: `USB_P`/`USB_N`, `CLK+`/`CLK-` (not with `CLK_N`), `D0P`/`D0N`, `USB_DP`/`USB_DM`, `DQS0_t`/`DQS0_c`, `Net-(U12-USB_D+)`/`Net-(U12-USB_D-)`, a lone `_P` with no partner.
- [ ] Port `net_queries.extract_diff_pair_base` (KiCadRoutingTools), cited in the docstring.
- [ ] Commit.
...
- [ ] Failing tests: two crossing airwires of partner nets count `pair_weight`; of non-partners, the lighter weight as now; `leaf_costs` native and Python equal with partners set.
- [ ] Implement in Python and Rust; rebuild the native module.
- [ ] Commit.
...
- [ ] Failing test: a crossed pair on decided parts gives one finding naming the pair and the two parts, with the swap/turn advice.
- [ ] Implement; skill line; migration note for the next release.
- [ ] Commit.

### Task 5: Measurement

- [ ] `fixtures/bench.py` against the baseline; tally in the commit.
- [ ] Full test suite.
